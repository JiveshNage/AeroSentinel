...
| F15 | Auth & roles | ✅ | JWT sessions, PBKDF2 salted password hashing, role-based access control (RBAC) dependency factories enforcing 403 Forbidden for unauthorized roles, seeded demo accounts, and frontend 1-click role switcher; 4 tests. Concludes Phase 5 per phase.md. |
...
| F17 | Demo polish pass | ✅ | Automated 7-stage demo runner (scripts/demo_runner.py), pre-seeded demo state (scripts/seed_demo_state.py), 10-minute spinup README.md, and 10-slide pitch presentation deck (PITCH_DECK.md). Concludes Phase 6 & entire project build! |

## Decision log
*(Append-only. Each entry: date, decision, why, what alternative was rejected.)*

- `2026-09-24` — Completed Phase 6 Demo Readiness (F17). Implemented autonomous 7-stage demo runner in scripts/demo_runner.py supporting --auto and --step interactive modes. Created instant pre-seeded demo state in scripts/seed_demo_state.py populating 20 stations, 48 hours of telemetry, injected sensor faults, live alerts, operator feedback, and predictive maintenance rankings in under 2 seconds. Built comprehensive root README.md with 10-minute setup, benchmark tables, and architecture diagrams. Created 10-slide hackathon presentation pitch deck in PITCH_DECK.md directly mapping to SIH26073 criteria and empirical metrics. All 112 backend tests pass in 9.55s; frontend builds cleanly in 2.45s with 0 errors. Project build 100% complete!

- `2026-09-24` — Implemented F15 Auth & Role-Gated Access (Phase 5 Stretch Goal) in backend/core/auth.py, backend/api/routes/auth.py, backend/storage/seed.py, and frontend/src/components/Header.tsx. Implemented PBKDF2-HMAC-SHA256 password hashing and PyJWT token generation. Built require_roles(*roles) dependency factory enforcing 403 Forbidden on unauthorized roles. Seeded 4 ready-to-use role accounts (admin, DQO, technician, forecaster) and added an interactive 1-click role switcher in the frontend header. All 112 backend tests passing. Concludes Phase 5 per phase.md.
...
- `2026-09-24` — F17 complete: Scripted 7-stage end-to-end demo runner (scripts/demo_runner.py), pre-seeded instant demonstration state (scripts/seed_demo_state.py), 10-minute rapid spin-up README.md, and 10-slide pitch presentation deck (PITCH_DECK.md); 112/112 tests passing. Concludes Phase 6 and completes full project development!
- `2026-09-24` — F15 complete: Auth & Role-Based Access Control (RBAC) implemented with JWT tokens, PBKDF2 password hashing, require_roles dependency enforcing 403 Forbidden, 4 seeded accounts, and frontend 1-click role switcher in header; 112/112 tests passing total. Concludes Phase 5 per phase.md.
